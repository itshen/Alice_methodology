...
最朴素的方案只有一个终止条件：LLM 不再调用工具。但这在生产环境里是不够的。LLM 可能进入工具调用的死循环——反复调用同一个工具、反复重试某个失败操作——从来不会"自然停下来"。所以需要迭代次数上限兜底。

但迭代次数上限本身也是一个两难：设太低，复杂任务被强制截断，用户体验差；设太高，失控的循环会烧掉大量 token 才被发现。没有一个对所有任务都对的数字。

更麻烦的是，迭代次数上限解决不了"用户中途想停"的问题。用户点击取消时，循环可能正卡在一个工具调用里等待结果。AbortSignal 的作用是穿透所有等待层，立刻让整个调用链干净退出。

而不可恢复错误是第四道防线——某些错误重试没有意义（上下文已损坏、模型返回非法结构、安全扫描失败），应该立即终止，而不是消耗更多轮次。

四个条件加在一起，才能覆盖"正常完成、超限终止、主动取消、异常退出"四种不同的收尾路径。每一条路径对应不同的善后逻辑——这也是为什么终止条件不能简化。
...
Agent 循环的出口设计成 AsyncGenerator 事件流，而不是回调或 Promise，背后有一个核心判断：**Agent 的执行过程不是一个"稍后完成"的操作，而是一个持续产生事件的过程**。

用回调实现并不难，但会把"控制流"和"事件处理"混在一起——调用方要同时管理"何时启动、何时取消"和"如何响应每个事件"两件事，代码很快变得难以推理。EventEmitter 解决了事件分发的问题，但引入了另一个问题：事件发射者不知道消费者处理得多快，快速发射的事件会在内存里积压。

AsyncGenerator 的好处是：消费者控制节奏，每次 `await next()` 才触发下一个事件，背压自然形成。取消也很干净，调用 `generator.return()` 整条调用链都会收到停止信号。

但这个选择也有代价。AsyncGenerator 在中间层包装时比较繁琐——如果需要把子 Agent 的事件流 merge 进父 Agent 的流，需要显式处理多流合并逻辑，比 EventEmitter 的广播模式复杂。调试 async generator 的执行顺序，在某些场景下也比 Promise 链更难追踪。

选 AsyncGenerator，本质上是在"调用方控制权"和"中间层灵活性"之间做了一个取舍。对于 Alice 这种"UI 是主要消费者"的场景，前者更重要。
